...
def volume_axis(*args, **kwargs):
    ...
    * ``def volume_axis(axis)``
    ...
    def axis_from_name(name):
        name = name.upper()
        if name == 'R':
            return np.asarray([0, 0], dtype=np.int)
        elif name == 'L':
            return np.asarray([0, 1], dtype=np.int)
        elif name == 'A':
            return np.asarray([1, 0], dtype=np.int)
        elif name == 'P':
            return np.asarray([1, 1], dtype=np.int)
        elif name == 'S':
            return np.asarray([2, 0], dtype=np.int)
        elif name == 'I':
            return np.asarray([2, 1], dtype=np.int)

    def axis_from_index(index, flipped=False):
        index = np.asarray(index).reshape([])
        flipped = np.asarray(flipped).reshape([])
        return np.asarray([index, flipped], dtype=np.int)

    def axis_from_axis(ax):
        ax = np.asarray(ax, dtype=np.int).flatten()
        if ax.size != 2:
...
def volume_layout(*args, **kwargs):
    ...
    def layout_from_index(index, flipped=False):
        ...
        ndim = index.shape[0]
        flipped = np.asarray(flipped, dtype=np.int).flatten()
...
def invert_permutation(perm):
    ...
    n = perm.shape[-1]
    k = perm.shape[0]
    identity = np.arange(n, dtype=np.int)[None, ...]
    identity = np.broadcast_to(identity, [k, n])  # Repeat without allocation
    iperm = np.empty_like(perm)
    iperm = np.put_along_axis(iperm, perm, identity, -1)
